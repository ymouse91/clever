...
function clickDie(color){
  // Laudan noppa (ei hopealautanen)
  const die = state.dice.find(d => d.color===color && d.alive && !d.inSilver);
  if (!die || die.value==null){ setStatus("Heitä ensin."); return; }

  if (color === "white"){
    // Älä vielä aseta areaa → kysy dialogilla
    state.chosenDie = { die, area: null, source: "board" };

    const dlg = qs("#white-choose");
    if (!dlg){ console.warn("#white-choose puuttuu"); return; }

    // nollaa vanhat handlerit (ettei kerry onclick:jä)
    ...
    dlg.querySelectorAll("button[data-area]").forEach(btn=>{
      btn.addEventListener("click", () => {
        ...
        state.chosenDie = { die, area, source: "board" };
        if (typeof highlightArea === "function") highlightArea(area);
        setStatus(`Valkoinen → ${area}. Valitse ruutu.`);
        // ÄLÄ kuluta mitään tässä; käyttäjä klikkaa kohderuutua ja
        // diePickedFinalize hoitaa kulutuksen vasta kun merkintä onnistuu.
      }, { once:true });
    });
    return;
  }
  
function onSilverDieClick(die){
  if (!die || !die.inSilver) return;

  // UI-kehys pysyy valkoisena, kunnes sijoitus onnistuu
  if (!state.ui) state.ui = {};
  state.ui.silverArmedDieId = String(die.id);
  document.querySelectorAll('#dice-silver .die').forEach(el=>{
    el.classList.toggle('armed', el.dataset.id === state.ui.silverArmedDieId);
  });

  if (die.color === "white"){
    state.chosenDie = { die, area: null, source: "silver" };
    ...
    dlg.querySelectorAll("button[data-area]").forEach(btn=>{
      btn.addEventListener("click", () => {
        dlg.close();
        const area = btn.dataset.area;
        state.chosenDie = { die, area, source: "silver" };
        if (typeof highlightArea === "function") highlightArea(area);
        setStatus(`Valkoinen (hopealautanen) → ${area}. Valitse ruutu.`);
      }, { once:true });
    });
    return;
  }

  // Värillinen hopeanoppa
  state.chosenDie = { die, area: die.color, source: "silver" };
  if (typeof highlightArea === "function") highlightArea(die.color);
  setStatus(`Hopealautanen: valitse ruutu alueelta ${die.color}.`);
}

  // Muut värit: suoraan alueeseen
  state.chosenDie = { die, area: color, source: "board" };
  if (typeof highlightArea === "function") highlightArea(color);
  setStatus(`Valitse ruutu alueelta: ${color}.`);
}


function routeWhiteTo(area){
  const white = state.dice.find(d=>d.color==="white" && d.alive && !d.inSilver);
  if (!white || white.value==null){ setStatus("Valkoista ei voi käyttää nyt."); return; }
  state.chosenDie = { die:white, area };
  routeDieToArea(white, area);
}

function routeDieToArea(die, area){
  // Visuaalinen kuittaus nopalle
  clearDieSelection();
  const el = [...document.querySelectorAll('#dice-available .die')]
    .find(d => d.classList.contains(die.color));
  if (el) el.classList.add("selected");

  // Statusviesti + mahdollinen korostus
  switch(area){
    case "yellow":
      setStatus(`Valittu ${prettyDie(die)} → Keltainen: napauta ruutua arvolla ${die.value}.`);
      state.chosenDie = {die, area};
      highlightYellowOptions(die.value);
      break;
    case "blue":
      setStatus(`Valittu ${prettyDie(die)} → Sininen: napauta summaa (B+W).`);
      state.chosenDie = {die, area};
      break;
    case "green":
      setStatus(`Valittu ${prettyDie(die)} → Vihreä: napauta seuraavaa hyväksyttävää solua.`);
      state.chosenDie = {die, area};
      break;
    case "orange":
      setStatus(`Valittu ${prettyDie(die)} → Oranssi: napauta seuraavaa tyhjää solua.`);
      state.chosenDie = {die, area};
      break;
    case "purple":
      setStatus(`Valittu ${prettyDie(die)} → Violetti: napauta seuraavaa sallittua solua.`);
      state.chosenDie = {die, area};
      break;
  }
}
...
function onSilverDieClick(die){
  if (!state.plusOneArmed){
    setStatus("Ota +1 käyttöön, jotta voit valita hopeanopan.");
    return;
  }
  armSilverDie(die); // tämä asettaa .armed-valkoisen kehyksen
}
...
// Merkitse hopealautasen noppa "armed"-tilaan (valkoinen kehys), ei kuluta +1
function armSilverDie(die){
  if (!die) return;

  // Tallenna “mikä noppa on aseistettu” UI:lle (kehys)
  if (!state.ui) state.ui = {};
  state.ui.silverArmedDieId = String(die.id);

  // Täytä CHOSEN so that tryMark* hyväksyy klikit
  // White = ei vielä kohdeväriä → kysytään alla
  state.chosenDie = {
    die,
    area: (die.color === "white") ? null : die.color, // blue/green/yellow/orange/purple
    source: "silver",                                 // meta: tulee hopealautaselta
  };

  // Valitse noppaan valkoinen kehys
  document.querySelectorAll('#dice-silver .die').forEach(el=>{
    el.classList.toggle('armed', el.dataset.id === state.ui.silverArmedDieId);
  });

  // Ohjeistus + kohdealueen korostus
  if (die.color === "white"){
    setStatus?.("Valkoinen: valitse kohdeväri tälle heitolle.");
    showWhiteTargetChooserFromSilver();  // ↓ luodaan alla
  } else {
    setStatus?.(`Valitse ruutu alueelta: ${die.color}.`);
    // jos sulla on alueiden highlightterit, kutsu ne tässä:
    if (typeof highlightArea === "function") highlightArea(die.color);
    // vaihtoehtoisesti tyhjennä muun värin highlightit, jos sinulla on sellainen:
    if (typeof clearYellowHighlights === "function") clearYellowHighlights();
  }

  // Päivitä nopat, jotta armed-kehys jää näkyviin
  ...
  qs("#dice-silver").addEventListener("click", (ev) => {
    ...
    if (die.color === "white"){
      ...
      dlg.querySelectorAll("button[data-area]").forEach(btn=>{
        btn.onclick = () => {
          dlg.close();
          state.chosenDie = { die, area: btn.dataset.area };
          renderDice();
          routeDieToArea(die, btn.dataset.area);
        };
      });
    } else {
...
function rowBonusLabel(code){
  switch(code){
    case "ORANGE_5":   return "→ Oranssi 5";
    case "REROLL":     return "→ ↻";
    case "GREEN_MARK": return "→ Vihreä ✓";
    case "PURPLE_6":   return "→ Violetti 6";
